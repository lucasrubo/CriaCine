...
const SearchBar = ({
  ...
  const generateSuggestions = useCallback(
    async (searchQuery: string) => {
      if (!searchQuery.trim()) {
        ...
        const tagSuggestions: SearchSuggestion[] = popularTags
          .slice(0, 4)
          .map((tag) => ({
            type: "tag",
            value: tag,
      ...
      try {
        ...
        if (
          searchQuery.toLowerCase().includes("lucas") ||
          searchQuery.toLowerCase().includes("ana")
        ) {
          newSuggestions.push({
            type: "user",
            value: searchQuery,
  ...
  const handleSuggestionClick = (suggestion: SearchSuggestion) => {
    ...
    if (suggestion.type === "post" && suggestion.data) {
      ...
      navigate(`/post/${suggestion.data.id}`);
    } else if (suggestion.type === "tag") {
      navigate(`/search?tag=${encodeURIComponent(suggestion.value)}`);
    } else {
